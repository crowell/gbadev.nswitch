...
s32 IosPatch_RUNTIME(bool wii, bool sciifii, bool vwii, bool verbose) {
	...
	if (AHBPROT_DISABLED) {
		disable_memory_protection();
			// IosPatch_AHBPROT copied here by Maxternal
		count += apply_patch("es_set_ahbprot", es_set_ahbprot_old, sizeof(es_set_ahbprot_old), es_set_ahbprot_patch, sizeof(es_set_ahbprot_patch), 25, verbose);
		if(wii)
		{
			if(verbose)
